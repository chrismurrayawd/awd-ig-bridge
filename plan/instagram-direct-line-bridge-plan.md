...
4. ~~**Prod-hardening scope:** ship the dev-grade bridge for the trial, or invest in the durable/retry version before real traffic?~~ **Resolved 2026-05-25 — defer.** Ship dev-grade for the Testers trial; do step 8 (durable store + retry/backoff + token refresh) after the dev-mode round-trip is proven and before Live-mode traffic (step 9). The relay/Direct Line code needs no change for the Azure-Bot/D365 topology — wiring is config-only.
